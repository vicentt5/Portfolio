...
if __name__ == "__main__":
    ...
    #vuelosdf4 = vuelosdf3.groupBy(col('vertical_rate')<0).count().filter(col('(vertical_rate < 0)')=='true').withColumnRenamed('(vertical_rate < 0)', 'estado').apply(lambda row: '1')
    #vuelosdf5 = vuelosdf3.groupBy(col('vertical_rate')>0).count().filter(col('(vertical_rate > 0)')=='true').withColumnRenamed('(vertical_rate > 0)', 'estado')
    vuelosdf4 = vuelosdf3.select(col('vertical_rate'))\
                .withColumn('vertical_rate', when(col('vertical_rate')<0,'-1')\
